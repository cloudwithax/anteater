...
# Exercises the batched Trash path (single _anteater_move_to_trash_batch call for
...
@test "remove_file_list batches eligible Trash moves into a single helper call" {
    ...
    # behavior by mv'ing each path into ANTEATER_TEST_TRASH_DIR. This lets the
    # test assert both "called once" and "every file landed in trash".
    run bash --noprofile --norc <<EOF
$(prelude)
_anteater_move_to_trash_batch() {
    mkdir -p "\$ANTEATER_TEST_TRASH_DIR"
    printf 'call %d\n' "\$#" >> "$count_file"
    local p dest
    for p in "\$@"; do
        dest="\$ANTEATER_TEST_TRASH_DIR/\$(basename "\$p").stub.\$RANDOM"
...
EOF
    ...
    local in_trash
    in_trash=$(find "$ANTEATER_TEST_TRASH_DIR" -type f | wc -l | tr -d ' ')
    [ "$in_trash" -eq 5 ]
    ...
    local ok_lines
    ok_lines=$(awk -F'\t' '$4 == "ok" && $2 == "trash"' "$ANTEATER_DELETE_LOG" | wc -l | tr -d ' ')
    [ "$ok_lines" -eq 5 ]
}
...
@test "remove_file_list falls through to per-file path when batch helper fails" {
    ...
    # Stub the batch helper to fail, and stub anteater_delete to record per-file
    # invocations and act on the file. This proves the fallback path runs once
    # per file rather than silently dropping the batch.
    run bash --noprofile --norc <<EOF
$(prelude)
_anteater_move_to_trash_batch() { return 1; }
anteater_delete() {
    printf 'anteater_delete %s\n' "\$1" >> "$trace"
    rm -f "\$1"
    return 0
}
remove_file_list "$list" "false"
EOF
    ...
    [ "$fallback_calls" -eq 2 ]
    grep -qF "anteater_delete $f1" "$trace"
    grep -qF "anteater_delete $f2" "$trace"
}

@test "_anteater_move_to_trash_batch returns 1 when trash CLI is missing under ANTEATER_TEST_NO_AUTH" {
    local f1="$SANDBOX/p.plist"
    : > "$f1"

    # Drop ANTEATER_TEST_TRASH_DIR so we exercise the real helper path; the
    # ANTEATER_TEST_NO_AUTH guard must fail closed before any AppleScript runs.
    run bash --noprofile --norc <<EOF
set -euo pipefail
export ANTEATER_TEST_NO_AUTH=1
unset ANTEATER_TEST_TRASH_DIR
source "$PROJECT_ROOT/lib/core/common.sh"
_anteater_move_to_trash_batch "$f1"
EOF
